...
def preprocess(X, RSSI_value_selection):
    ...
    # X, features_name = square_features(X, features_name)

    X.drop('Device_ID', axis=1, inplace=True)
    return X[features_name]

def square_features(X, features):
    l_features = features.copy()
    for col in l_features:
        X['square_'+col] = X[col]**2
        features += ['square_'+col]
    return X, features
...
def train_create_features(data, window_size, stride):
    ...
    # X_features, train_feat = square_features(X_features, train_feat)
